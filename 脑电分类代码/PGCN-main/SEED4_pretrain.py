...
def eeg_data(label_list, trial_list, raw_path, path):
    ...
    for subject in os.listdir(raw_path):
        subject_name = str(subject).strip('.mat')   #从文件名中移除 .mat 扩展名，得到受试者的名称
        #初始化空的 NumPy 数组来存储数据和标签，设置变量 clip 为 None，设置 frequency 为 "de_LDS"（可能表示使用了差分熵和线性动态系统方法提取的特征），并初始化计数器 count。
        ...
        for i in trial_list:
            dataKey = frequency + str(i+1)       #构造数据键，将 frequency 字符串和试验索引加一后的字符串连接起来
            # print(dataKey)
            ...
            # print(metaData)
            trMetaData = einops.rearrange(metaData, 'w h c -> h w c')  # (42,62,5)    使用 einops.rearrange 函数重新排列 metaData 数组的形状
            # print(trMetaData)
            #增加计数器，将 trMetaData 转换为 NumPy 数组，并根据 label_list 中的标签创建一个标签数组。
            count += 1
            x = np.array(trMetaData)
            y = np.array([label_list[i],] * x.shape[0])

            #如果 data 数组为空，则直接赋值；如果不为空，则将新数据追加到 data 数组中，并将新标签追加到 label 数组中。
            if data.shape[0] == 0:
                data = x
                label = y
            else:
            ...
            if count == 16:  #如果处理了 16 个试验，记录此时 data 数组的形状。
                clip = data.shape[0]
        ...
        np.save(os.path.join(path, (str(subject).strip('.mat') + ".npy")), dict)  #将字典保存为 .npy 文件，文件名由受试者名称和 .npy 扩展名组成，并保存到 path 指定的目录。
...
if __name__ == "__main__":
    ...
    data_path = "../npy_data/seed4/" # 存储合并的两种数据   定义了保存合并后数据的路径，这是将要保存 .npy 文件的文件夹

    session = 1     #设置会话编号为 1，表示接下来要处理的是第一个会话的数据
    label_list = [1, 2, 3, 0, 2, 0, 0, 1, 0, 1, 2, 1, 1, 1, 2, 3, 2, 2, 3, 3, 0, 3, 0, 3]    #定义了一个标签列表，包含了第一个会话中每个试验的情感标签
    # 选取每种类型的后2个trial用于测试
